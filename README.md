...
## 8. Logo files
The `logo/` folder has the current brand logo:

- `cedars-logo.png` — full logo (icon + "Cedars" + "AGENCY" + tagline), transparent background. Used in the nav and footer.
- `cedars-icon.png` — cropped icon-only version (no text), used as the favicon and app icon, since the full logo is too detailed to read at 16–32px.

This logo has a fixed dark-green color with no light-mode variant, so it has poor contrast against the dark-mode background (~1.9:1, below the recommended 3:1 minimum for graphics). `styles.css` gives it a light backing plate in dark mode only (`html[data-theme="dark"] .logo`) so the artwork itself stays untouched but stays visible. If you get a version of the logo in a lighter color, that workaround can be removed.

To swap in a different logo later, replace `cedars-logo.png` (and re-crop `cedars-icon.png` if needed) and everything — nav, footer, favicon, social preview image, structured data — updates automatically since they all reference the same two filenames.

## 9. Security — what's already built in, and what to keep doing
**Already in the code:**
